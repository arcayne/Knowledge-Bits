...
test('approve A delivers the exact immutable package and persists the adapter response before verification', async () => {
  ...
  const result = await fixture.service.run(fixture.deliveryId);
...
test('retry A preserves immutable package identity and external idempotency after an already imported response', async () => {
  ...
  const first = await fixture.service.run(fixture.deliveryId);
  fixture.adapter.nextStatus = 'already_imported';
  const second = await fixture.service.run(fixture.deliveryId);
...
test('invalidating A and approving B creates a different immutable delivery and external key', async () => {
  const fixture = await approvedFixture('A');
  await fixture.service.run(fixture.deliveryId);
  ...
  await fixture.service.run(deliveryB.id);
...
test('rejects delivery without a matching current approval', async () => {
  ...
  await assert.rejects(new DeliveryService({ repository, adapter }).run(delivery.id), /approved checksum/i);
...
test('transient, permanent schema, and verification failures preserve the immutable package', async () => {
  ...
  assert.equal((await transient.service.run(transient.deliveryId)).state, 'waiting');
  ...
  assert.equal((await permanent.service.run(permanent.deliveryId)).state, 'needs_human');
  ...
  assert.equal((await verify.service.run(verify.deliveryId)).state, 'failed');
  ...
  assert.equal((await verifyError.service.run(verifyError.deliveryId)).state, 'waiting');
...
async function approvedFixture(variant: string) {
  const clock = () => new Date('2026-07-13T10:00:00.000Z');

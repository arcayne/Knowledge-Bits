...
ReviewPackageService now decides whether editorial findings are advisory only after assembling the package. The warning path requires a materialized `nuglet.lesson.v1` target at schema version `1.1.0`. A legacy package with a blocking editorial finding retains the hard approval issue and exposes no advisory-only warning. The existing workflow may still complete its asset job; the restored policy applies when the immutable review package is assembled for approval.
...
- `pnpm --filter @knowledge-bits/api exec tsx --test src/services/review-packages.test.ts` failed before the ReviewPackageService scope repair because a legacy blocking editorial finding still allowed a decision.
...
- `pnpm --filter @knowledge-bits/api exec tsx --test src/services/review-packages.test.ts` passed, 15 tests, including materialized Story/Playbook warning and legacy hard-block coverage.
...
- `pnpm --filter @knowledge-bits/api typecheck && pnpm --filter @knowledge-bits/api test` passed, 114 tests.
...
The validated repair is committed on `codex/nuglet-generation-v03`.
